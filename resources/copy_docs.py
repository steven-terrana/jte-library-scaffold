...
# Move library index
if (os.path.exists('libraries/index.md')):
  file = open("/docs/libraries/index.md").read()
  with mkdocs_gen_files.open(f"libraries/index.md", "w") as f:
    print(f"{file}\n\n## Available Libraries\n\n{overview_table}", file=f)

# Create Libraries section navigation using mkdocs-awesome-pages
pages = {
  "nav": [ lib["file"] for lib in libraries ] 
}
